...
// by MistWeb is if an infinite loop is introduced (e.g. via useEffect), so we
...
export async function request<T>(method: string, endpoint: string, options?: RequestOptions): Promise<APIResponse<T>> {
  ...
  if (endpoint !== "ws/start" && method === "POST" && (syncNode.startsWith("http:") || !syncNode.startsWith("https://mist.anti.money"))) {
    notification.warning({
      message: "INSECURE API REQUEST",
      description: "Your wallet password has been compromised.",
      duration: 30,
      btn: React.createElement("a", { href: "https://github.com/MistDrop/MistWeb/issues/new?labels=server%20connection%20issues", target: "_blank", rel: "noopener noreferrer" },
