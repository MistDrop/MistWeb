...
import { makeV2Address } from "@utils/mist";

import { localeSort } from "@utils";

/** Finds a wallet in the wallet map by the given Mist address. */
...
export async function calculateAddress(
  ...
  if (walletOrFormat === undefined || typeof walletOrFormat === "string") {
    ...
    const format: WalletFormatName = walletOrFormat || "mistwallet";

    const privatekey = await applyWalletFormat(format || "mistwallet", password, username);
