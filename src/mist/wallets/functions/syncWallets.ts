...
import { MistAddressWithNames, lookupAddresses } from "../../api/lookup";
...
function syncWalletProperties(
  wallet: Wallet,
  address: MistAddressWithNames | null,
  syncTime: Date
): Wallet {
  if (address) {
    return {
...
export async function syncWallet(
  ...
  const mistAddress = lookupResults[address];
  syncWalletUpdate(wallet, mistAddress, dontSave);
}
...
export async function syncWallets(): Promise<void> {
  ...
  const updatedWallets = Object.entries(wallets).map(([_, wallet]) => {
    const mistAddress = lookupResults[wallet.address];
    if (!mistAddress) return wallet; // Skip unsyncable wallets
    return syncWalletProperties(wallet, mistAddress, syncTime);
